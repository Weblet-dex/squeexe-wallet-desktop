...
Item
{
    ...
    onPair_supportedChanged: if (!pair_supported) webEngineViewPlaceHolderAg.visible = false
    ...
    function loadChart(right_ticker, left_ticker, force = false, source="livecoinwatch")
    {
        ...
        let widget_x = (380 - 4)
        let widget_y = (200 - 3)
        let scale_x = rootag.width / widget_x
        let scale_y = rootag.height / widget_y
        ...
        if (source == "livecoinwatch")
        {
            ...
            if (rel_ticker != "" && base_ticker != "")
            {
                ...
                {
                    webEngineViewPlaceHolderAg.visible = true
                    return
                }
                chart_html = `
                <style>
        ...
        dashboard.agWebEngn.loadHtml(chart_html)
    }
    ...
    {
        anchors.fill: parent
        visible: !webEngineViewPlaceHolderAg.visible
    ...
    Item
    {
        id: webEngineViewPlaceHolderAg
        anchors.fill: parent
        visible: false

        Component.onCompleted:
        {
            dashboard.agWebEngn.parent = webEngineViewPlaceHolderAg
            dashboard.agWebEngn.anchors.fill = webEngineViewPlaceHolderAg
        }
        Component.onDestruction:
        {
            dashboard.agWebEngn.visible = false
            dashboard.agWebEngn.stop()
        }
        onVisibleChanged: dashboard.agWebEngn.visible = visible

        Connections
        {
            target: dashboard. agWebEngn
            ...
            function onLoadingChanged(webEngineLoadReq)
            {
                ...
                else webEngineViewPlaceHolderAg.visible = false
            }
        }
    }
    ...
    //         {
    //             rootag.loadChart(left, right)
    //         }
    //         else
    //         {
    //             rootag.loadChart(right, left)
    //         }
    //     }
    // }
