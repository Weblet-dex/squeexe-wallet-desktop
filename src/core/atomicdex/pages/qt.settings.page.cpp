...
namespace atomic_dex
{
    ...
    void atomic_dex::settings_page::set_current_lang(QString new_lang)
    {
        ...
        SPDLOG_INFO("Locale before parsing Squeexe Wallet settings: {}", QLocale().name().toStdString());
        QLocale::setDefault(get_locale(new_lang.toStdString()));
        SPDLOG_INFO("Locale after parsing Squeexe Wallet settings: {}", QLocale().name().toStdString());
